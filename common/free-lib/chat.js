...
class chat {
	...
	async handleOnRecall(message) {
		...
		let name = message.chat_type !== 'group' ? '对方' : `"${list[index].from_name}" `
	...
	send(message, onProgress = false) {
		return new Promise(async (result, rejects) => {
			// 添加到消息历史记录
			let {
				k
			...
			// 验证是否上线
			if (!this.checkOnline()) return rejects('未上线')
			// 上传文件
	...
	// 删除指定聊天记录
	async deleteChatDetailIten(message, isSend = true) {
		// 获取对方id
		let id = message.chat_type === 'user' ? (isSend ? message.to_id : message.from_id) : message.to_id
		// key值：chatDetail_当前用户id_会话类型_接收人/群id
		let key = `chatDetail_${this.user.id}_${message.chat_type}_${id}`
	...
	async updateChatItem(where, data) {
		...
		let isNewdata = -1
		if (where.list) {
			let newdata = where.list[where.list.length - 1].id
			isNewdata = newdata === where.chatId
		}

		let isNoreadnum = list[index].noreadnumAll.indexOf(where.chatId)
	...
	formatChatItemData(message, isSend) {
		...
		data = (isSend || message.chat_type !== 'group' || message.type === 'system') ? data :
